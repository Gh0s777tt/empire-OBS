...
QString OBSBasic::GetUrlDisplayName(QUrl url)
{
	QUrlQuery query = QUrlQuery(url.query(QUrl::FullyEncoded));
	return query.hasQueryItem("layer-name") ? query.queryItemValue("layer-name", QUrl::FullyDecoded) : url.host();
}

void OBSBasic::AddDropURL(QUrl url, obs_data_t *settings, const obs_video_info &ovi)
{
	QUrlQuery query = QUrlQuery(url.query(QUrl::FullyEncoded));
...
void OBSBasic::AddDropSource(const DropItem &dropItem)
{
	...
	case DropType_Html:
		...
		break;
	case DropType_Url:
		AddDropURL(QUrl(dropItem.url), settings, ovi);
		break;
	}
			...
			for (int i = 0; i < urls.size(); i++) {
				...
				if (!fileInfo.exists()) {
					...
					if (ConfirmDropUrl(url.url())) {
						addList << DropItem{url.url(), GetUrlDisplayName(url), DropType_Url};
					}
					continue;
				}
				...
				if (fileInfo.suffix().compare("url", Qt::CaseInsensitive) == 0) {
					...
					if (ConfirmDropUrl(urlTarget))
						addList << DropItem{urlTarget, GetUrlDisplayName(QUrl(urlTarget)),
								    DropType_Url};
					continue;
