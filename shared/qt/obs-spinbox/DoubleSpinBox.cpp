...
void DoubleSpinBox::clampCursorPosition()
{
	...
	if (!suffix().isEmpty()) {
		...
		if (cursorPosition > suffixIndex) {
			if (hasSelectedText && lineEdit()->selectionEnd() > suffixIndex) {
				lineEdit()->setSelection(lineEdit()->selectionStart(), lineEdit()->text().length());
			} else {
	...
	if (!prefix().isEmpty()) {
		...
		if (cursorPosition < prefixIndex) {
			if (hasSelectedText && lineEdit()->selectionStart() < prefixIndex) {
				lineEdit()->setSelection(0, lineEdit()->selectionEnd());
			} else {
